...
def _send_verification_email(user, request):
    ...
    send_mail("Verify your Bookify email", f"Click to verify: {link}", None, [user.email])
        ...
        if form.is_valid():
            ...
            if user is None:
                ...
                if not user.is_email_verified:
                    return render(request, "accounts/verify_prompt.html", {"email": user.email})
                login(request, user); return redirect("post_login")
    else:
        form = LoginForm()
...
def verify_email_view(request):
    ...
    if not user:
        return render(request, "accounts/error.html", {"message": "Invalid or expired verification link."})
...
def post_login_router(request):
    ...
    if role == User.Roles.CUSTOMER: return redirect("customer_dashboard")
    if role == User.Roles.OWNER: return redirect("owner_dashboard")
    if role == User.Roles.STAFF: return redirect("staff_dashboard")
    return HttpResponseForbidden("Unknown role")

def is_owner(u): return u.is_authenticated and u.role == User.Roles.OWNER
...
def create_invitation_view(request):
    if request.method == "POST":
        ...
        send_mail("Your Bookify staff invite", f"Open this link to join: {link}", None, [email])
...
def accept_invite_view(request):
    ...
    if not inv or not inv.is_valid():
        return render(request, "accounts/error.html", {"message": "Invalid or expired invitation."})
    if request.method == "POST":
        pwd = request.POST.get("password")
        if pwd and len(pwd) >= 8 and any(c.isalpha() for c in pwd) and any(c.isdigit() for c in pwd):
            user = User.objects.create_user(email=inv.email, password=pwd, role=User.Roles.STAFF, is_email_verified=True)
            inv.accepted_at = timezone.now(); inv.save()
...
def customer_dashboard(request):
    if request.user.role != User.Roles.CUSTOMER: return HttpResponseForbidden("403")
    return render(request, "accounts/dashboard_customer.html")

@login_required
def owner_dashboard(request):
    if request.user.role != User.Roles.OWNER: return HttpResponseForbidden("403")
    return render(request, "accounts/dashboard_owner.html")

@login_required
def staff_dashboard(request):
    if request.user.role != User.Roles.STAFF: return HttpResponseForbidden("403")
